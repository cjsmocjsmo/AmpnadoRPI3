...
class CreateMyXML:
    ...
    def CreatAlbumViewXML(self, data):
        ...
        # songlist = []
        for s in data["Songs"]:
            albumViewXML.append("\t\t <Songname>{}</Songname> \n".format(s[0]))
            albumViewXML.append("\t\t <SongId>{}</SongId> \n".format(s[1]))
            # songitem = [
            #     "\t\t <Songname>{}</Songname> \n".format(s[0]),
            #     "\t\t <SongId>{}</SongId> \n".format(s[1]),
            # ]
    ...
    def CreatArtistViewXML(self, data):
        ...
        for a in data["Albums"]:
            artistViewXML.append("\tt <Album>{}</Album> \n".format(a[0]))
            artistViewXML.append("\tt <AlbumId>{}</AlbumId> \n".format(a[1]))
            # artistitem = [
            #     "\tt <Album>{}</Album> \n".format(a[0]),
            #     "\tt <AlbumId>{}</AlbumId> \n".format(a[1])
            # ]
    ...
    def CreateArtAlphaXML(self, data):
        ...
        for s in data["artalpha"]:
            artalphaXML.append("\t <item>{}</item> \n".format(s))
        artalphaXML.append("</ArtAlpha> \n")
        return artalphaXML
    ...
    def CreateAlbAlphaXML(self, data):
        ...
        for s in data["albalpha"]:
            albalphaXML.append("\t <item>{}</item> \n".format(s))
        albalphaXML.append("</AlbAlpha> \n")
        return albalphaXML
    ...
    def CreateSongAlphaXML(self, data):
        ...
        for s in data["songalpha"]:
            songalphaXML.append("\t <item>{}</item> \n".format(s))
        songalphaXML.append("</SongAlpha> \n")
        return songalphaXML
